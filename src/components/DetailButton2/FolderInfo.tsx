...
interface SidebarProps {
    ...
    onClose: () => void;
    selectedFolderPermission?: selectedFolderPermissionInter[];
    parentFolderName: string
}
const FolderInfo: React.FC<SidebarProps> = ({ isOpen, folderInfo, onClose,selectedFolderPermission,parentFolderName }) => { 
          ...
          {folderInfo ? (
            <div className="flex flex-col w-full h-full mt-5 font min-h-screen">
              <div className="flex-[1] flex gap-1 border-b border-gray-300 font-bold text-[18px]">
                <img src="/foldericon.svg" alt="" className='h-[24px] w-[24px]'/>
                <div className="text-1xl text-black">{folderInfo.name}</div>
              </div>
              <div className="flex-[9] flex flex-col">
                <div className="border-b border-gray-300 flex flex-col gap-3 pt-5">
                  ...
                  <div className="flex flex-col">
                    <div className='text-black'>Người có quyền truy cập</div>
                    <div>avatar</div>
                  </div>
                  <button className="w-[70%] py-2 rounded-2xl border mb-5 hover:bg-gray-200 text-black">
                ...
                <div className="pt-5 flex flex-col gap-10">
                  <div className="flex flex-col">
                    ...
                    <div className="flex">
                      ...
                      <div className="flex-[6] text-black">{folderInfo.name}</div>
                    </div>
                    <div className="flex">
                    ...
                    <div className="flex">
                      ...
                      <div className="flex-[6] text-black">{(folderInfo.total_size*0.000001).toFixed(4)}MB</div>
                    </div>
                    <div className="flex">
                      <div className="flex-[4] text-gray-400">Modified</div>
                      <div className="flex-[6] text-black">{folderInfo.updated_at.substring(0,10)}</div>
                    </div>
                    {/* <div className="flex">
                      <div className="flex-[4] text-gray-400">Type</div>
                      <div className="flex-[6]">{folderInfo.storage_detail.mime_type.split(".").pop()?.toLowerCase()}</div>
                    </div> */}
                    <div className="flex">
                      <div className="flex-[4] text-gray-400">Uploaded by</div>
                      <div className="flex-[6] text-black">{selectedFolderPermission && selectedFolderPermission.length > 0 
                          ? selectedFolderPermission[0].user?.email 
                          : 'Unknown'}</div>
                    </div>
                    <div className="flex">
                      <div className="flex-[4] text-gray-400">Date uploaded</div>
                      <div className="flex-[6] text-black">{folderInfo.created_at.slice(0,10)}</div>
                    </div>
                    <div className="flex">
                      <div className="flex-[4] text-gray-400">Description</div>
                      <div className="flex-[6] text-black">{folderInfo.description}</div>
                    </div>
                  </div>
